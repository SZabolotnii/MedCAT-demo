...
- переглянути таблицю розпізнаних сутностей від MedCAT (канонічний ківорд, текстове оточення, кластер та знайдені value-хінти);
- переглянути окрему таблицю HintNER (векторні підказки: канонічний ківорд, збіг у тексті, кластер);
- ознайомитись із сирим JSON-виводом MedCAT та підсвіченим текстом.
...
- `PyYAML` - парсинг конфігурації free-text правил (`data/hc.yaml`)
...
- Кластери з `String` у назві та всі записи, що надходять зі `source = numerical`, вимагають додаткового value-хінта поруч із ключовим словом.
- Для числових Ківордів перелік допустимих діапазонів зберігається у `data/numerical_model.json` — позамежні значення автоматично відкидаються.
- Текстові значення та синоніми для таких сутностей беруться з `data/internal.csv` (`data_value`, `data_hints`), тож підтримуйте актуальність цих полів під час оновлення онтології.
- Комбіновані препарати (з токенами на кшталт `aspirin / dipyridamole`) повертаються лише якщо в тексті присутні всі складові; просте згадування базового слова більше не активує декілька CUI.
- Винятки (ключові слова/кластери, які можуть витягатись без value-хінта) задаються у `data/hc.yaml` (`ft_value_without_hint_by_keyword`, `ft_value_without_hint_by_cluster`); оновлюйте цей файл для розширення білого списку.
- У JSON-відповідь (`result['entities']`) додається поле `value_hints`: список із типом (`numeric`/`text`), значенням та позиціями знайденого value-хінта; це дає змогу відображати користувачу, що саме підтвердило сутність.
